...
export class CaseService {
  ...
  private readonly api = `${environment.apiUrl}/api/cases`;
  ...
  closeCase(id: number): Observable<Case> {
    return this.http.patch<Case>(`${this.api}/${id}/close`, {});
  }

  getQrCode(id: number): Observable<{ qrCode: string }> {
    return this.http.get<{ qrCode: string }>(`${this.api}/${id}/qr`);
  }

  exportCasePdf(id: number): Observable<Blob> {
  ...
  updateVersionStatus(caseId: number, versionId: number, status: string): Observable<CaseVersion> {
    return this.http.patch<CaseVersion>(`${this.api}/${caseId}/versions/${versionId}/status`, { status });
  }
  ...
  addComment(caseId: number, versionId: number, request: AddCommentRequest): Observable<VersionComment> {
    return this.http.post<VersionComment>(
      `${this.api}/${caseId}/versions/${versionId}/comments`, request
    );
  }
  ...
  removeCollaborator(caseId: number, collaboratorId: number): Observable<void> {
    return this.http.delete<void>(`${this.api}/${caseId}/collaborators/${collaboratorId}`);
  }

  // ── RELATIONS ──────────────────────────────────────────────────────

  addRelation(caseId: number, targetCaseId: number, relationType: string): Observable<CaseRelation> {
    return this.http.post<CaseRelation>(`${this.api}/${caseId}/relations`, { targetCaseId, relationType });
  }
